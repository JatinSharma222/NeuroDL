...
const APIRequest = ({ image, patientId = null }) => {
  ...
  const connectSocket = async () => {
    ...
    socket.on("progress", (data) => {
      ...
      setSteps(prev => prev.map(s => {
        ...
        const dur = status === "done" && stepStartRef.current[step]
          ? (Date.now() - stepStartRef.current[step]) / 1000
          : s.duration;
        return { ...s, status, message: message || s.message, duration: dur ?? s.duration };
      }));
    });
    ...
    return new Promise((resolve) => {
      if (socket.connected) { resolve(socket.id); return; }
  ...
  const sendRequest = async () => {
    ...
    try {
      const res  = await authFetch(`${API_URL}/predict`, { method: "POST", body: formData });
  ...
  return (
    <div className="space-y-8 mt-8">
      ...
      {loading && (
        <InferenceProgress steps={steps} elapsed={elapsed} />
      )}
        ...
        <div className="fade-in space-y-8">
          ...
          <div className="text-center">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-sm">
              <span className="badge badge-primary">Confidence: {response.confidence}</span>
              <span className="badge badge-info">Model: {response.model_used}</span>
              <span className="badge badge-success">Accuracy: {response.model_accuracy}</span>
              {response.gradcam_performed    && <span className="badge badge-success">✓ Grad-CAM</span>}
              {response.segmentation_performed && <span className="badge badge-info">✓ Segmentation</span>}
              {response.scan_id    && <span className="badge" style={{ background: "var(--color-bg-tertiary)", color: "var(--color-text-secondary)" }}>Scan #{response.scan_id}</span>}
              {response.patient_id && <span className="badge" style={{ background: "#f0fdf4", color: "#15803d", border: "1px solid #bbf7d0" }}>Patient #{response.patient_id}</span>}
            </div>
          </div>
